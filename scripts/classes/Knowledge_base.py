...
class Knowledge_base():
    ...
    def create_local_link(self, obj_1, obj_2):
        ...
        # не получится ли при этом ситуации, когда понядобятся обновленные
        # поля класса ?
        ...
        # а перед добалением искать возможную дублирующую пару (для (a, b)
        # искать (b, a) и не добалять вторую)
    ...
    def create_def(self, class_name, list_words):
        ...
        if not self.class_exist(class_name):
            ...
            # и exit(-1) нельзя дать т к этот код может вполняться не только в
            # субпроцессе
