...
# В kb должны сохраняться группы графов,
# такие как глобальный, локальный, ссылки на питоновские программы.
# Операции над графами в классе граф.
# Функции выводов графов заключаются в переопределении методов __str__.
...
class Knowledge_base():
    """Класс для хранения графов и операций над ними."""

    def __init__(self):
        """."""
        # дискуссионный вопрос о том, какой тип структуры должен быть.
        # точно не словарь, т к дубли по названию не допустимы
        # операции над множествами не планируются
        # поэтому остается список

        # в принятые решения
        # удобно иметь как разделение на классы и объекты
        ...
        self.path_python_program = os.path.join(
            path_to_workspace,
            'knowledge_base',
            'python_program'
        )
    ...
    # внутри всех методов в kb делать проверку/преобразование из строки в
    # объект
    ...
    # obj py def link  можно перегрузить по типу данных функции, но в питоне с
    # этим проблемы

    # принятое решение № 20, 21
    # принятые решения хотя бы раз сюда продублировать или хотя бы самому
    # прочитать, для освежения в памяти
    # сделать перегрузку по типу строка и по готовому объекту
    ...
    def create_local_link(self, obj_1, obj_2):
        ...
        # не получится ли при этом ситуации, когда понадобятся обновленные
        # поля класса ?

        # возможная проверка на недопущение дублей пар
        # есть проблема, но это не точно что set setов делать нельзя
        # по хорошему, local_links сделать множеством, т к оно больше,
        # а перед добавлением искать возможную дублирующую пару (для (a, b)
        # искать (b, a) и не добавлять вторую)
        ...
        # проверка на существование элемента не нужна
        # т к из за разного времени ввода экземпляры класса объект всегда
        # будут разные
        # даже если делать проверку, то через оператор in, а не через set в
        # общем списке связей

        # хранить ссылки нужно именно в классах, т к в них все поля содержатся
    ...
    def create_def(self, class_name, list_words):
        """."""
        # во все create нужно добавить проверку на существование создаваемого
        # объекта
        ...
        if not self.class_exist(class_name):
            ...
            # сейчас субпроцесс, поэтому все валится в ноды,
            # а должно в отдельный поток ошибок
            # и exit(-1) нельзя дать т к этот код может выполняться не только в
            # субпроцессе
